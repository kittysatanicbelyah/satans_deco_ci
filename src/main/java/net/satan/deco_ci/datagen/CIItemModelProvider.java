...
public class CIItemModelProvider extends ItemModelProvider {
    ...
    private ItemModelBuilder curtainBlockItem(DeferredBlock<Block> block) {
        return withExistingParent(block.getId().getPath(),
                ResourceLocation.withDefaultNamespace("item/generated"))
                .texture("layer0", ResourceLocation.fromNamespaceAndPath(satans_deco_ci.MODID, "block/" + block.getId().getPath() + "_single_lower"));
    }

    private ItemModelBuilder simpleBlockItem(DeferredBlock<Block> block) {
        return withExistingParent(block.getId().getPath(),
                ResourceLocation.withDefaultNamespace("item/generated"))
                .texture("layer0", ResourceLocation.fromNamespaceAndPath(satans_deco_ci.MODID, "block/" + block.getId().getPath()));
    }

    private ItemModelBuilder curtain1BlockItem(DeferredBlock<Block> block) {
        return withExistingParent(block.getId().getPath(),
                ResourceLocation.withDefaultNamespace("item/generated"))
                .texture("layer0", ResourceLocation.fromNamespaceAndPath(satans_deco_ci.MODID, "block/" + block.getId().getPath() + "_lower"));
    }

    private ItemModelBuilder pelmetSolidBlockItem(DeferredBlock<Block> block){
        ResourceLocation pelmet = CISupGen.matchTexture(block.getId().getPath(), CISupGen.isWooden(block));
        return withExistingParent(block.getId().getPath(),
                ResourceLocation.fromNamespaceAndPath(satans_deco_ci.MODID, "block/pelmet/pelmet_solid")).texture("pelmet", pelmet);
    }

    private ItemModelBuilder pelmetCarvedBlockItem(DeferredBlock<Block> block){
        ResourceLocation pelmet = CISupGen.matchTexture(block.getId().getPath(), true);
        return withExistingParent(block.getId().getPath(),
                ResourceLocation.fromNamespaceAndPath(satans_deco_ci.MODID, "block/pelmet/pelmet_carved")).texture("pelmet", pelmet);
    }

    private ItemModelBuilder pelmetSoftBlockItem(DeferredBlock<Block> block){
        ResourceLocation pelmet = CISupGen.matchCurtain(block.getId().getPath(), false);
        return withExistingParent(block.getId().getPath(),
                ResourceLocation.fromNamespaceAndPath(satans_deco_ci.MODID, "block/pelmet/pelmet_soft")).texture("pelmet", pelmet);
    }

    private ItemModelBuilder pelmetOrnateBlockItem(DeferredBlock<Block> block){
        ...
        return withExistingParent(block.getId().getPath(),
                ResourceLocation.fromNamespaceAndPath(satans_deco_ci.MODID, "block/pelmet/pelmet_ornate")).texture("pelmet", pelmet)
                .texture("metal", metal).texture("ornament", ornament);
    }

    private ItemModelBuilder pelmetOrnateTinselBlockItem(DeferredBlock<Block> block){
        ResourceLocation pelmet = CISupGen.matchCurtain(block.getId().getPath(), true);
        ResourceLocation metal = CISupGen.matchOrnament(block.getId().getPath(), true);
        ResourceLocation ornament = CISupGen.matchOrnament(block.getId().getPath(), false);
        ResourceLocation tinsel = CISupGen.matchTinsel(block.getId().getPath(), true);
        return withExistingParent(block.getId().getPath(),
                ResourceLocation.fromNamespaceAndPath(satans_deco_ci.MODID, "block/pelmet/pelmet_ornate_tinsel")).texture("pelmet", pelmet)
