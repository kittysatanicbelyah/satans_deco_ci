...
public interface CISupGen {
    ...
    static boolean isWooden(DeferredBlock<Block> block) {
       boolean wood = false;
    ...
    static ResourceLocation matchTexture(String name, boolean isWooden) {
        ResourceLocation res = ResourceLocation.withDefaultNamespace("block/vine");
        if (isWooden) {
            boolean oak = false;
            for (int x = 0; x < SupportedWood.length; x++) {
                if (name.contains(SupportedWood[x])) {
                    ...
                    if (!oak) {
                        res = ResourceLocation.withDefaultNamespace("block/" + SupportedWood[x] + "_planks");
                        break;
                    }
                }
            }
            ...
            for (int x = 0; x < SupportedColors.length; x++) {
                if (name.contains(SupportedColors[x])) {
                    ...
                    if (!color) {
                        res = ResourceLocation.withDefaultNamespace("block/" + SupportedColors[x] + "_wool");
                        break;
                    }
                }
            }
    ...
    static ResourceLocation matchCurtain(String name, boolean isOrnate) {
        ...
        for (int x = 0; x < SupportedColors.length; x++) {
            if (name.contains(SupportedColors[x])) {
                ...
                if (!color) {
                    res = isOrnate ? ResourceLocation.fromNamespaceAndPath(satans_deco_ci.MODID, "block/pelmet_ornate_" + SupportedColors[x])
                            : ResourceLocation.fromNamespaceAndPath(satans_deco_ci.MODID, "block/curtain_default_" + SupportedColors[x] + "_lower");
                    break;
                }
            }
        }
    ...
    static ResourceLocation matchOrnament(String name, boolean isMetal) {
        ...
        for(int x = 0; x<PelmetOrnates.length; x++) {
            if (name.contains(PelmetOrnates[x])) {
                res = isMetal ? ResourceLocation.withDefaultNamespace("block/" + PelmetOrnates[x] + "_block")
            : ResourceLocation.fromNamespaceAndPath(satans_deco_ci.MODID, "block/pelmet_ornate_" + PelmetOrnates[x]);
                break;
            }
        }
        return res;
    ...
    static ResourceLocation matchTinsel(String name, boolean doOpposites){
        ...
        for (int x = 0; x < SupportedColors.length; x++) {
            if (name.contains(SupportedColors[x])) {
                ...
                if (!color) {
                    res = doOpposites ?  ResourceLocation.fromNamespaceAndPath(satans_deco_ci.MODID, "block/pelmet_ornate_tinsel_" + COLOR_DICHOTOMY.get(SupportedColors[x]))
                : ResourceLocation.fromNamespaceAndPath(satans_deco_ci.MODID, "block/pelmet_ornate_tinsel_" + SupportedColors[x]);
                    break;
                }
            }
        }
